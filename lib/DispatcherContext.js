...
DispatcherContext.prototype.getDomain = function getDomain(name) {
    var domainName = name;
    if (!this.domainInstances[domainName]) {
        var Domain = this.dispatcher.domains[domainName];
        if (!Domain) {
            throw new Error('Domain ' + domainName + ' was not registered.');
        }
        this.domainInstances[domainName] = this.dispatcher.domains[domainName];
    }
    return this.domainInstances[domainName];
};
...
DispatcherContext.prototype.dehydrate = function dehydrate() {
    ...
    Object.keys(self.dispatcher.domains).forEach(function domainInstancesEach(domainName) {
        var domain = self.dispatcher.domains[domainName];
        domains[domainName] = domain.dehydrate();
    });
...
DispatcherContext.prototype.rehydrate = function rehydrate(dispatcherState) {
    ...
    if (dispatcherState.domains) {
        Object.keys(dispatcherState.domains).forEach(function domainStateEach(domainName) {
            var state = dispatcherState.domains[domainName],
                domain = self.getDomain(domainName);
            if (domain.rehydrate) {
                domain.rehydrate(state);
            }
        });
    }
};
