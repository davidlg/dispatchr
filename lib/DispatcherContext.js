...
DispatcherContext.prototype.getDomain = function getDomain(name) {
    var storeName = name;
    if (!this.domainInstances[storeName]) {
        var Store = this.dispatcher.domains[storeName];
        if (!Store) {
            throw new Error('Store ' + storeName + ' was not registered.');
        }
        this.domainInstances[storeName] = this.dispatcher.domains[storeName];
    }
    return this.domainInstances[storeName];
};
...
DispatcherContext.prototype.dehydrate = function dehydrate() {
    var self = this,
        ...
        console.log("storeInstances", self.storeInstances);
        console.log("domainInstances", this.dispatcher.domains);

    Object.keys(self.dispatcher.domains).forEach(function domainInstancesEach(storeName) {
        console.log("keys:", storeName);
        var store = self.dispatcher.domains[storeName];
        /*if (!store.dehydrate || (store.shouldDehydrate && !store.shouldDehydrate())) {
            console.log("me salgo!");
            return;
        }*/
        console.log("dehydrate:", store.dehydrate());
        domains[storeName] = store.dehydrate();
...
DispatcherContext.prototype.rehydrate = function rehydrate(dispatcherState) {
    ...
    if (dispatcherState.domains) {
        Object.keys(dispatcherState.domains).forEach(function domainStateEach(storeName) {
            var state = dispatcherState.domains[storeName],
                store = self.getDomain(storeName);
            if (store.rehydrate) {
                store.rehydrate(state);
            }
        });
