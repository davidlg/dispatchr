...
Dispatcher.prototype.registerDomain = function registerDomain(store) {
   ...
   //}
    ...
    if (this.domains[storeName]) {
        if (this.domains[storeName] === store) {
            // Store is already registered, nothing to do
            return;
        }
        throw new Error('Store with name `' + storeName + '` has already been registered.');
    }
    this.domains[storeName] = store;
...
Dispatcher.prototype.isRegisteredDomain = function isRegisteredDomain(store) {
    var storeName = this.getStoreName(store),
        storeInstance = this.domains[storeName];
